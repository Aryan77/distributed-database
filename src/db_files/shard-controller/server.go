...
func (sc *ShardCtrler) apply(v raft2.ApplyMsg) {
...
func (sc *ShardCtrler) Raft() *raft2.Raft {
	return sc.rf
}

func StartServer(servers []*labrpc.ClientEnd, me int, persister *raft2.Persister) *ShardCtrler {
	sc := new(ShardCtrler)
	sc.me = me
	...
	sc.applyCh = make(chan raft2.ApplyMsg)
	sc.rf = raft2.Make(servers, me, persister, sc.applyCh)
...
func (sc *ShardCtrler) Debug(format string, a ...interface{}) {
	...
	prefix := fmt.Sprintf("%s%s S%d %s[SERVER] ", preamble, raft2.Microseconds(time.Now()), sc.me, epilogue)
